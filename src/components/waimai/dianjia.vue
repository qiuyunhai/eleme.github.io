	...
	<div class="dianjia-item">
		...
		<div class="secend">
			...
			<div v-for="(items,index) in item.others" :key="index">
				<div class="small-size" v-if="index<=1">
					<span class="brand">{{items.name}}</span> {{items.content}} <span v-if="index==0 && item.others.length>2" class="iconfont fr" @click="hideshow=!hideshow" :class="[hideshow ? 'icon-youjiantou' : 'icon-xiasanjiao']"></span>
				</div>
			</div>
...
<script>
	export default {
		...
		watch: {
			hideshow: function() {
				if(this.hideshow) {
					this.height = 20 * (this.item.others.length - 2);
				} else {
					this.height = 0;
				}
			}
